...
const PopupModule = ({ isOpen, onClose, title, content, onConfirm, onCancel }) => {
    ...
    return (
        <div className={`popup-overlay ${isOpen ? "show" : "hide"}`} onClick={onClose}>
            <div className={`popup-container ${isOpen ? "show" : "hide"}`} onClick={(e) => e.stopPropagation()}>
...
export const usePopup = () => {
    ...
    const showPopup = (title, content, onConfirm = null, onCancel = null) => {
        setPopupState({
            ...
            title,
            content,
            onConfirm: onConfirm ? () => { onConfirm(); closePopup(); } : null,
            onCancel: onCancel ? () => { onCancel(); closePopup(); } : null,
        });
    };
